...
export async function PATCH(request) {
    ...
    try {
        ...
        if (!messageId || !status) {
            return NextResponse.json({ message: "Message ID and status are required." }, { status: 400 });
        }
        ...
        const result = await messageCollection.updateOne({ _id: new ObjectId(messageId) }, { $set: { status: status } });
